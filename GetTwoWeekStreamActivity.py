...
def createLine(username, extraData, line, inactive=False):
  ...
  if not inactive:
    if '🟢' in line:
      if len(line.split('|')) > 3: lang = line.split('|')[4]
      if len(line.split('|')) > 3: cat = line.split('|')[5]
      if extraData:
